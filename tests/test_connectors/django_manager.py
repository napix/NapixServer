...
class TestResourceFieldGeneration(unittest2.TestCase):
    ...
    def test_auto_genration(self):
        ...
        self.assertHasKeys(
            cls, ['id', 'name', 'owner', 'max_speed', 'created'])
        self.assertFalse(any(cls.resource_fields[x]['computed']
                         for x in ['name', 'owner', 'max_speed']))
        self.assertFalse(
            any(cls.resource_fields[x]['optional'] for x in ['name', 'max_speed']))
        self.assertTrue(cls.resource_fields['created']['computed'])
        self.assertTrue(cls.resource_fields['id']['computed'])
        self.assertEqual(
            cls.resource_fields['name']['description'], 'name of the var')
    ...
    def test_custom_resource_fields(self):
        ...
        self.assertHasKeys(
            cls, ['id', 'name', 'owner', 'max_speed', 'created', 'color'])
        self.assertEqual(
            cls.resource_fields['name']['description'], 'name of the var')
        self.assertEqual(cls.resource_fields['name']['example'], 'clio')

        self.assertEqual(cls.resource_fields['max_speed'][
                         'description'], 'The maximum speed ever')
