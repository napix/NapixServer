...
class ConversationPlugin(object):
    ...
    This plugins ensure all the responses are made in JSON,
    ...
    def unwrap(self, request):
        ...
        if int(request.get('CONTENT_LENGTH') or 0) != 0:
            if ('CONTENT_TYPE' in request and
                ...
                try:
                    ...
                    self.logger.debug(
                        'JSON object is %s', request.body.getvalue())
                    raise bottle.HTTPError(400, 'Unable to load JSON object',
...
<p>
Anyway if you just want to explore the Napix server when it's in DEBUG mode,
use the <a href="?authok">?authok GET parameter</a>
</p>
