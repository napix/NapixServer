...
Here are the base classes for the implementations of the backends used by :mod:`napixd.store`
...
Its initilized with the parameters either from the configuration or from the user.
A Backend is reusable and may generate multiple Stores.

The :class:`BaseBackend` class may be used as a starting point but it's not required.
In simple cases, the backend may just be a function that returns a Store class

The Store
---------

The :class:`Store instance<napixd.store.Store>` is the object handled to the user.
...
Asynchronous implementations requires the user to call :meth:`.save<BaseStore.save>`
...
Synchronous implementations directly commit the modifications to the underlying persistance support.
...
class BaseBackend( object):
    ...
    ``config`` come either from the config file or is manually specified by the user.
    """
