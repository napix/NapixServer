...
class Story(object):
    ...
    def append_title(self, string, *args, **kwargs):
        ...
        if kwargs:
            raise TypeError, 'Got unexpected keywork arguments : %s' % ','.join( kwargs.keys())
        title = string % args
...
class Autodocument(object):
    ...
    def make_documentation( self, alias, manager):
        doc_file_path = os.path.join( self.directory, manager.__module__ + '.rst' )
        logger.debug('Start dumping doc of %s to %s', manager.get_name(), doc_file_path )
        try:
            handle = open( doc_file_path, 'wb' )
        except IOError as e:
            logger.error( 'Failed to write %s, because of %s', doc_file_path, str(e))
            raise
        body = Story()
    ...
    def _document( self, manager, body):
        ...
        for field in manager.resource_fields:
            validate_methods[ 'validate_resource_' + field] = (field, 'Check the validity of `%s` field' % field)
